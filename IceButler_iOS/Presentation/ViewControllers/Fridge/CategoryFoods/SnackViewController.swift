...
class SnackViewController: UIViewController {
    ...
    private func setupObserver() {
        FridgeViewModel.shared.isChangeAllFoodList(foodListIdx:7) {
            self.foodCollectionView.reloadData()
            self.noFoodLabel.isHidden = true
            self.foodCollectionView.isHidden = false
        }
        
        FridgeViewModel.shared.isNoFoodList(foodListIdx: 7) {
...
extension SnackViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return FridgeViewModel.shared.allFoodListCount(foodListIdx: 7)
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "FoodCell", for: indexPath) as! FoodCell
        
        FridgeViewModel.shared.allFoodListFoodDday(foodListIdx: 7, index: indexPath.row, store: &cell.cancellabels) { foodDday in
            cell.setDday(foodDday: foodDday)
        }
        
        FridgeViewModel.shared.allFoodListFoodName(foodListIdx: 7, index: indexPath.row, store: &cell.cancellabels) { foodName in
            cell.setFoodName(foodName: foodName)
        }
        
        FridgeViewModel.shared.allFoodListFoodImage(foodListIdx: 7, index: indexPath.row, store: &cell.cancellabels) { foodImage in
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let foodIdx = FridgeViewModel.shared.foodIdx(foodListIdx: 7, index: indexPath.row)
