...
class DrinkViewController: UIViewController {
    ...
    private func setupObserver() {
        FridgeViewModel.shared.isChangeAllFoodList(foodListIdx:4) {
            self.foodCollectionView.reloadData()
            self.noFoodLabel.isHidden = true
            self.foodCollectionView.isHidden = false
        }
        
        FridgeViewModel.shared.isNoFoodList(foodListIdx: 4) {
...
extension DrinkViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return FridgeViewModel.shared.allFoodListCount(foodListIdx: 4)
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "FoodCell", for: indexPath) as! FoodCell
        
        FridgeViewModel.shared.allFoodListFoodDday(foodListIdx: 4, index: indexPath.row, store: &cell.cancellabels) { foodDday in
        ...
        cell.setFoodIdx(foodIdx: FridgeViewModel.shared.foodIdx(foodListIdx:4, index: indexPath.row))
        
        return cell
    }
    
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let foodIdx = FridgeViewModel.shared.foodIdx(foodListIdx: 4, index: indexPath.row)
