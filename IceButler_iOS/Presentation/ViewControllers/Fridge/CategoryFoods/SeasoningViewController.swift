...
class SeasoningViewController: UIViewController {
    ...
    private func setupObserver() {
        FridgeViewModel.shared.isChangeAllFoodList(foodListIdx:8) {
            self.foodCollectionView.reloadData()
            self.noFoodLabel.isHidden = true
            self.foodCollectionView.isHidden = false
        }
        
        FridgeViewModel.shared.isNoFoodList(foodListIdx: 8) {
...
extension SeasoningViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return FridgeViewModel.shared.allFoodListCount(foodListIdx: 8)
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "FoodCell", for: indexPath) as! FoodCell
        
        FridgeViewModel.shared.allFoodListFoodDday(foodListIdx: 8, index: indexPath.row, store: &cell.cancellabels) { foodDday in
            cell.setDday(foodDday: foodDday)
        }
        
        FridgeViewModel.shared.allFoodListFoodName(foodListIdx: 8, index: indexPath.row, store: &cell.cancellabels) { foodName in
        ...
        cell.setFoodIdx(foodIdx: FridgeViewModel.shared.foodIdx(foodListIdx:8, index: indexPath.row))
        
        return cell
    }
    
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let foodIdx = FridgeViewModel.shared.foodIdx(foodListIdx: 8, index: indexPath.row)
